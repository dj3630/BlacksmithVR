...
public class ReticlePoser_Intro : MonoBehaviour
{
    ...
    protected virtual void LateUpdate()
    {
        ...
        if (result.isValid)
        {
            if (targetReticle != null)
            {
                ...
                if (FindObjectOfType<StartSelect_Intro>().isClickAvailable)
                {
                    hammer.transform.position = result.worldPosition + new Vector3(0.1F, -0.3F, 0.4F);
                    hammer.transform.rotation = hammerPose.rotation;
                }

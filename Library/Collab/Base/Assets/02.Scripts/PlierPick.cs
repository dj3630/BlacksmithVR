...
public class PlierPick_1 : MonoBehaviour
{
    ...
    public void PickManager()
    {
        ...
        {
            if (Physics.CheckBox(plierTr.position, Vector3.one * 0.2F, Quaternion.identity, 1 << 8, QueryTriggerInteraction.Collide))
            {
                pickedObj = Physics.OverlapBox(plierTr.position, Vector3.one * 0.2F, Quaternion.identity, 1 << 8, QueryTriggerInteraction.Collide)[0].transform;
                Pick();
            }
            else return;
        }
